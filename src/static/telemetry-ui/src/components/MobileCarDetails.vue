<template>
      ...
      <div class="telemetry-list">
        <draggable 
          ...
          <template #item="{ element, index }">
            <!-- The entire telemetry item toggles expansion on click -->
            <div class="telemetry-item" :class="{ alt: index % 2 === 1 }" @click="toggleExpanded(element)">
              <!-- Always visible row -->
              <div class="row-main">
  ...
  export default {
    ...
    computed: {
      // Look up the current car based on the route param "id"
      currentCar() {
    ...
    watch: {
      // When currentCar changes, load telemetry order.
      currentCar: {
        handler(newCar, oldCar) {
          // Reset expansion state only when the car changes.
